...
public class UsuarioService {
    ...
    public List<Usuario> buscarUsuarios(String termino) {
        ...
        return usuarioRepository
                .findByNombreContainingIgnoreCaseOrApellidoPaternoContainingIgnoreCaseOrCorreoContainingIgnoreCase(
                        termino,
                        termino,
                        termino);
    }
    ...
    /**
     * Usuarios que pueden recibir una herramienta en préstamo.
     * Los perfiles de cliente no forman parte de este catálogo operativo.
     */
    @Transactional(readOnly = true)
    ...
    public Usuario obtenerUsuarioAsignableAPrestamo(Integer idUsuario) {
        ...
        if (!usuarioRolRepository.existsUsuarioAsignableAPrestamo(idUsuario)) {
            throw new IllegalArgumentException(
                    "El usuario seleccionado debe tener rol ADMIN o GUIA.");
        }
    ...
    /**
     * Catálogo seguro para las altas administrativas de reservaciones.
     * Sólo los usuarios con rol CLIENTE y estado Activo pueden ser seleccionados.
     */
    @Transactional(readOnly = true)
    ...
    public Usuario obtenerClienteActivoParaReservacion(Integer idUsuario) {
        ...
        if (idUsuario == null) {
            throw new IllegalArgumentException(
                    "Debe seleccionar un cliente activo válido.");
        }
        ...
        if (!usuarioRolRepository.existsClienteActivoParaReservacion(idUsuario)) {
            throw new IllegalArgumentException(
                    "El usuario seleccionado debe tener rol CLIENTE y estar activo.");
        }
    ...
    public Usuario registrarCliente(RegistroForm registroForm) {
        ...
        Estado estadoActivo
                = estadoRepository
                        .findByNombreEstadoIgnoreCase("Activo")
                        .orElseThrow(() -> new IllegalStateException(
                        "No existe el estado Activo requerido para registrar usuarios."));

        Rol rolCliente
                = rolRepository
                        .findByRolIgnoreCase("CLIENTE")
                        .orElseThrow(() -> new IllegalStateException(
                        "No existe el rol CLIENTE requerido para registrar usuarios."));
        ...
        usuario.setApellidoPaterno(
                registroForm.getApellidoPaterno().trim());

        usuario.setApellidoMaterno(
                limpiarOpcional(registroForm.getApellidoMaterno()));

        usuario.setCorreo(
                registroForm.getCorreo().trim());

        usuario.setTelefono(
                limpiarOpcional(registroForm.getTelefono()));
    ...
    @Transactional
    public Usuario cambiarPasswordTemporal(
            String correo,
    ...
    /**
     * Exposes the server-side rule so MVC forms can present a binding error
     * before invoking {@link #save(com.Proyecto_Grupo_1.domain.Usuario)}.
     */
